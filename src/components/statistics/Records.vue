      ...
      >
        ...
        <template v-slot:header.score="{ header }">
          ...
          <v-menu offset-y :close-on-content-click="false">
            <template v-slot:activator="{ on, attrs }">
              <v-btn icon v-bind="attrs" v-on="on">
                <v-icon small :color="score ? 'primary' : ''">
                  mdi-filter
                </v-icon>
              </v-btn>
            </template>
            <div style="background-color: white; width: 280px">
              <v-text-field v-model="score" class="pa-4" type="number" label="Enter the min score"></v-text-field>
              <v-btn @click="score = ''" small text color="primary" class="ml-2 mb-2">Clean</v-btn>
            </div>
          </v-menu>
        </template>
        ...
        return [
          ...
          { text: 'Class', value: 'class',
            filter: value => {
              return value != null &&
                      this.classs != null &&
                      value.toString().indexOf(this.classs) !== -1
            },
            width: '100px' },
          ...
          { text: 'Archer', value: 'archer',
            filter: value => {
              return value != null &&
                      this.archer != null &&
                      value.toString().indexOf(this.archer) !== -1
            },
            width: '175px' },
          { text: 'Event', value: 'event',
            filter: value => {
              return value != null &&
                      this.event != null &&
                      value.toString().indexOf(this.event) !== -1
            }
          },
          { text: 'national/EU/W', value: 'scope',
            filter: value => {
              return value != null &&
                      this.scope != null &&
                      value.toString().indexOf(this.scope) !== -1
            }
          },
